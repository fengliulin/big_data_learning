package cc.chengheng.casus.flow;

import cc.chengheng.combiner.MyCombiner;
import cc.chengheng.combiner.WordCountMapper;
import cc.chengheng.combiner.WordCountReducer;
...
public class JobMain extends Configured implements Tool {
    ...
    public int run(String[] args) throws Exception {
        // 1、创建一个job任务对象
        Job job = Job.getInstance(getConf(), "mapreduce_flowcount");
...
//        TextInputFormat.addInputPath(job, new Path("hdfs://192.168.1.100:8020/wordcount"));
        TextInputFormat.addInputPath(job, new Path("file:///Users/fengliulin/Documents/flowcount_input"));

        // 第二步：指定map阶段的处理方式
        job.setMapperClass(FlowCountMapper.class);
        job.setMapOutputKeyClass(Text.class); // 设置Map阶段的 k2 类型
        job.setMapOutputValueClass(FlowBean.class); // 设置Map阶段的 v2 类型
